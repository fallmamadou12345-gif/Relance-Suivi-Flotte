...
export default function Dashboard({ user }: { user?: any }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (isFirebaseConfigured) {
        try {
          unsubDrivers = onSnapshot(getCompanyCollection("drivers"), (snap) => {
          ...
          unsubUsers = onSnapshot(getCompanyCollection("users"), (snap) => {
            if (!snap.empty) {
          ...
          unsubLogs = onSnapshot(query(getCompanyCollection("logs"), orderBy("time", "desc"), limit(100)), (snap) => {
            if (!snap.empty) {
              setGlobalLogs(snap.docs.map(doc => doc.data()));
            }
          });

          unsubSettings = onSnapshot(getCompanyCollection("settings"), (snap) => {
            snap.forEach(doc => {
              const d = doc.data();
  ...
  useEffect(() => {
    ...
    const save = async () => {
      ...
      if (firebaseActive) {
        try {
          await setDoc(getCompanyDoc("settings", "sessions"), { data: agentSessions });
          await setDoc(getCompanyDoc("settings", "recruits"), { data: recruits });
          await setDoc(getCompanyDoc("settings", "reactivations"), { list: reactivations });
          await setDoc(getCompanyDoc("settings", "snapshot"), { data: lastCommandSnapshot });
          await setDoc(getCompanyDoc("settings", "fleets"), { list: fleets });
          await setDoc(getCompanyDoc("settings", "stats"), { totalCalls });
          await setDoc(getCompanyDoc("settings", "history"), { list: importHistory });
  ...
  const handleConfirm = useCallback(({ agent, comment, outcome }: any) => {
    ...
    if (firebaseActive) {
      addDoc(getCompanyCollection("logs"), logEntry).catch(console.error);
    }
    ...
    if (firebaseActive) {
      setDoc(getCompanyDoc("drivers", driver.id), updatedDriver).catch(console.error);
    }
  ...
  const onComment = useCallback((id: string, comment: string) => {
    setDrivers(ds => ds.map(d => {
      if (d.id === id) {
        ...
        if (firebaseActive) {
          setDoc(getCompanyDoc("drivers", id), updated).catch(console.error);
        }
        return updated;
      }
      return d;
  ...
  const handleFileUpload = (e: any) => {
    ...
    reader.onload = async (evt) => {
      ...
      (async () => {
        ...
        } catch (e) { console.warn("Local storage quota exceeded for drivers", e); }
        ...
        } catch (e) { console.warn("Local storage quota exceeded for metadata", e); }
        ...
        try {
          ...
          if (firebaseActive) {
            ...
            // Sync metadata
            await setDoc(getCompanyDoc("settings", "history"), { list: newHist });
            await setDoc(getCompanyDoc("settings", "recruits"), { data: updatedRecruits });
            await setDoc(getCompanyDoc("settings", "reactivations"), { list: updatedReactivations });
            await setDoc(getCompanyDoc("settings", "snapshot"), { data: newSnapshot });
            ...
            for (let i = 0; i < finalDrivers.length; i += chunkSize) {
              ...
              chunk.forEach(d => {
                const ref = getCompanyDoc("drivers", d.id);
                batch.set(ref, d);
              });
              await batch.commit();
            }
  ...
  const handleSyncToCloud = async () => {
    ...
    try {
      ...
      // Sync drivers
      for (const d of drivers) {
        await setDoc(getCompanyDoc("drivers", d.id), d);
      }
      // Sync users
      for (const u of users) {
        // Use name as ID for users if no ID
        const userId = u.id || u.name.replace(/\s/g, "_");
        await setDoc(getCompanyDoc("users", userId), u);
      }
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              ...
              {currentAgent && (
                <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 14px", background: (userRole === "SUPER_ADMIN" || userRole === "ADMIN_PARC") ? "#fef3c7" : "#eff6ff", borderRadius: 10, border: `1px solid ${(userRole === "SUPER_ADMIN" || userRole === "ADMIN_PARC") ? "#fcd34d" : "#bfdbfe"}`, fontSize: 13, color: (userRole === "SUPER_ADMIN" || userRole === "ADMIN_PARC") ? "#92400e" : "#1d4ed8", fontWeight: 700 }}>
                  <span style={{ opacity: 0.6, fontWeight: 500, marginRight: 4 }}>{user?.email}</span> | 
                  ...
                  }} title="Déconnexion" style={{ marginLeft: 8, border: "none", background: "transparent", cursor: "pointer", fontSize: 10, opacity: 0.6 }}>❌</button>
                </div>
              )}
            </div>
